...
import {
  ...
  Users, CalendarClock, ClipboardList, ShieldCheck, UserX, MapPin
} from 'lucide-react';
...
// ─── Stable sub-components (outside main component to prevent re-mount on state change) ───
 ...
 *  3 = # of assistants
 *  4 = Fixed rhythm (yes/no)
 *  5 = Who tracks hours (employer / assistant)
...
export function EmployerOnboarding({ onComplete }: Props) {
  ...
  const canNext = () => {
    ...
    if (step === 3) return apCount !== '';
    if (step === 4) return rhythm !== '';
    if (step === 5) return tracker !== '' && (tracker === 'employer' || approvalNeeded !== '');
    return false;
  };
  ...
  const renderStep = () => {
    ...
    if (step === 3) return (
      <div className="space-y-4">
        <h3 className="text-xl font-bold flex items-center gap-2"><Users className="w-6 h-6 text-primary" />Wie viele Assistenzpersonen?</h3>
        <div className="grid grid-cols-2 gap-3">
          {['1', '2', '3-5', '6+'].map(v => (
            <Radio key={v} active={apCount === v} onClick={() => setApCount(v)}
              label={v === '6+' ? '6 oder mehr' : v === '3-5' ? '3 – 5' : `${v} ${v === '1' ? 'Person' : 'Personen'}`} />
          ))}
        </div>
      </div>
    );
    ...
    if (step === 4) return (
      <div className="space-y-4">
        <h3 className="text-xl font-bold flex items-center gap-2"><CalendarClock className="w-6 h-6 text-primary" />Sind die Einsätze regelmässig?</h3>
        <p className="text-base text-muted-foreground">Jede Woche ungefähr gleich?</p>
        <Radio active={rhythm === 'yes'} onClick={() => setRhythm('yes')} label="Ja, fester Wochenrhythmus" />
        <Radio active={rhythm === 'no'} onClick={() => setRhythm('no')} label="Nein, wechselnd" />
      </div>
    );
